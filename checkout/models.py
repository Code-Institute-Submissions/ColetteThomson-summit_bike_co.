...
class Order(models.Model):
    ...
    # use set_null to keep order history, even if user profile is deleted
    ...
    def update_total(self):
        ...
        """ use Sum function across all line item totals for each line item.
            add 'or 0' to prevent error if all line items manually deleted
            from order, so setting order_total to zero instead of 'none' """
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.order_number:
            # if no order number, generate a new order number
...
class OrderLineItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        self.lineitem_total = round((self.product.price_now * self.quantity), 2)
        # then execute original save method
        super().save(*args, **kwargs)
